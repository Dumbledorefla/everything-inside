...
interface CarouselIdea {
  tema: string;
  sugestao_roteiro: string;
}

interface CarouselIdeaGeneratorProps {
  projectId: string;
  onIdeaSelected: (idea: CarouselIdea) => void;
}

export function CarouselIdeaGenerator({ projectId, onIdeaSelected }: CarouselIdeaGeneratorProps) {
  const [topic, setTopic] = useState("");
  const [ideas, setIdeas] = useState<CarouselIdea[]>([]);

  const { mutate: generateIdeas, isPending } = useMutation({
    mutationFn: async () => {
      if (!topic.trim()) throw new Error("Por favor, insira um tópico.");
      const { data, error } = await supabase.functions.invoke("idea-generator", {
        body: { projectId, topic, pieceType: "carousel" },
      });
      if (error) throw new Error(error.message);
      if (!data?.ideias || data.ideias.length === 0) throw new Error("A IA não retornou ideias.");
      return data.ideias as CarouselIdea[];
    },
    onSuccess: (data) => {
      setIdeas(data);
      toast.success("5 ideias de carrossel geradas!");
    },
    onError: (err: any) => toast.error(err.message || "Erro ao gerar ideias."),
  });

  return (
    <div className="p-4 space-y-4 bg-card/50 rounded-lg border border-border/20">
      <h3 className="text-sm font-semibold flex items-center gap-2">
        <Wand2 className="h-4 w-4 text-primary" /> Ideias para Carrossel
      </h3>
      <Textarea
        value={topic}
          ...
          {ideas.map((idea, index) => (
            <div key={index} className="p-3 rounded-md border border-border/15 bg-background/30 flex items-start justify-between gap-2">
              <div className="flex-1">
                <p className="font-semibold text-xs">{idea.tema}</p>
                <p className="text-xs text-muted-foreground/80 mt-1">{idea.sugestao_roteiro}</p>
              </div>
