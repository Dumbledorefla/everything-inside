...
import {
  Flame, Type, User, Layers, Cpu, Upload, X, Loader2,
  Star, Zap, Crown, ChevronDown, ChevronUp, Lock, Download
} from "lucide-react";
...
const MODELS: Record<StudioMode, ModelOption[]> = {
  ...
  lora: [
    { id: "fal-ai/flux-lora", name: "FLUX LoRA (fal.ai)", provider: "fal", cost: "premium", quality: "máxima", description: "Treina o modelo com suas fotos. Identidade 100% fiel.", requiresTraining: true },
    { id: "together/lora", name: "Together LoRA", provider: "together", cost: "premium", quality: "máxima", description: "LoRA na together.ai. Identidade completa e consistente.", requiresTraining: true },
  ],
};

const MODE_INFO: Record<StudioMode, { label: string; icon: any; description: string; requiresPhoto: boolean }> = {
  text2img: { label: "Texto → Imagem", icon: Type, description: "Descreva a cena em texto e gere imagens de alta qualidade.", requiresPhoto: false },
  ip_adapter: { label: "IP-Adapter", icon: User, description: "Envie uma foto. O modelo preserva sua identidade facial na imagem gerada.", requiresPhoto: true },
  ...
  lora: { label: "LoRA Treinado", icon: Cpu, description: "Use um modelo treinado com suas fotos para identidade 100% fiel.", requiresPhoto: false },
};
...
function ResultCard({ url, onDownload }: { url: string; onDownload: (url: string) => void }) {
  return (
    <motion.div
      ...
      className="relative group rounded-xl overflow-hidden border border-border/50 bg-secondary/30"
    >
...
export default function ProStudio() {
  ...
  const handleGenerate = async () => {
    if (!user || !activeProjectId) { toast.error("Selecione um projeto primeiro"); return; }
    if (!prompt.trim()) { toast.error("Escreva um prompt antes de gerar"); return; }
    if (MODE_INFO[activeMode].requiresPhoto && !referencePhoto) { toast.error("Envie uma foto de referência para este modo"); return; }
    if (activeMode === "lora" && !loraModelId.trim()) { toast.error("Informe o ID do modelo LoRA treinado"); return; }

    setIsGenerating(true);
    setResults([]);
    try {
      const { data, error } = await supabase.functions.invoke("pro-studio", {
        body: {
      ...
      const urls = (data?.results ?? []).filter((r: any) => r.status === "completed" && r.url).map((r: any) => r.url);
      if (urls.length > 0) { setResults(urls); toast.success(`${urls.length} imagem(ns) gerada(s) com sucesso`); }
      else { toast.error(data?.results?.[0]?.error ?? "Nenhuma imagem foi gerada"); }
    } catch (err: any) {
  ...
  const handleDownload = (url: string) => {
    ...
    a.href = url;
    a.download = `pro-studio-${Date.now()}.png`;
    a.target = "_blank";
    a.click();
  };
  ...
  if (!activeProjectId) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 text-center p-8">
        ...
        <p className="text-muted-foreground">Selecione um projeto para usar o Pro Studio.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-border/50">
        ...
        <div>
          <h1 className="text-lg font-bold text-foreground">Pro Studio</h1>
          <p className="text-xs text-muted-foreground">Geração profissional de imagens com múltiplos provedores</p>
        </div>
        <div className="ml-auto">
          <Badge variant="outline" className="text-red-400 border-red-500/30 bg-red-500/5 text-xs">PRO</Badge>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-80 flex-shrink-0 border-r border-border/50 overflow-y-auto p-4 space-y-5">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(Object.entries(MODE_INFO) as [StudioMode, typeof MODE_INFO[StudioMode]][]).map(([mode, info]) => {
                ...
                return (
                  <button key={mode} onClick={() => handleModeChange(mode)} className={cn(
                    "flex flex-col items-start gap-1 p-3 rounded-lg border text-left transition-all",
                    activeMode === mode ? "border-red-500/50 bg-red-500/10 text-red-300" : "border-border/50 bg-secondary/30 text-muted-foreground hover:border-border hover:text-foreground"
                  )}>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {currentModels.map((model) => (
                <button key={model.id} onClick={() => !model.requiresTraining && setSelectedModel(model.id)} className={cn(
                  "w-full flex flex-col gap-1.5 p-3 rounded-lg border text-left transition-all",
                  selectedModel === model.id ? "border-red-500/50 bg-red-500/10" : "border-border/50 bg-secondary/30 hover:border-border",
            ...
            <div className="space-y-2">
              ...
              {referencePhoto ? (
                <div className="relative rounded-lg overflow-hidden border border-border/50">
                  ...
                  <button onClick={() => setReferencePhoto(null)} className="absolute top-2 right-2 p-1 rounded-full bg-black/60 hover:bg-black/80 text-white">
                    <X className="h-3 w-3" />
                  </button>
                </div>
              ) : (
                <div onDrop={handleFileDrop} onDragOver={(e) => e.preventDefault()} onClick={() => fileInputRef.current?.click()}
                  ...
                  {uploadingPhoto ? <Loader2 className="h-6 w-6 text-muted-foreground animate-spin" /> : (
                    <>
              ...
              <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) handlePhotoUpload(file); }} />
            </div>
          )}
          ...
          {activeMode === "lora" && (
            <div className="space-y-2">
              ...
              <input type="text" value={loraModelId} onChange={(e) => setLoraModelId(e.target.value)} placeholder="Ex: https://huggingface.co/seu-lora"
              ...
              <p className="text-[10px] text-muted-foreground">Cole o URL do modelo LoRA treinado (HuggingFace, fal.ai, etc.)</p>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {(["1:1", "9:16", "16:9"] as Ratio[]).map((r) => (
                <button key={r} onClick={() => setRatio(r)} className={cn(
                  "flex-1 py-1.5 rounded-lg border text-xs font-medium transition-all",
                  ratio === r ? "border-red-500/50 bg-red-500/10 text-red-300" : "border-border/50 bg-secondary/30 text-muted-foreground hover:border-border"
                )}>{r}</button>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setNumVariations(Math.max(1, numVariations - 1))} className="p-1.5 rounded-lg border border-border/50 bg-secondary/30 hover:bg-secondary text-muted-foreground">
                <ChevronDown className="h-3 w-3" />
              </button>
              <span className="text-sm font-bold text-foreground w-6 text-center">{numVariations}</span>
              <button onClick={() => setNumVariations(Math.min(4, numVariations + 1))} className="p-1.5 rounded-lg border border-border/50 bg-secondary/30 hover:bg-secondary text-muted-foreground">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-border/50 space-y-3">
            ...
            <Button onClick={handleGenerate} disabled={isGenerating || !prompt.trim()} className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold">
              {isGenerating ? (<><Loader2 className="h-4 w-4 mr-2 animate-spin" />Gerando...</>) : (<><Flame className="h-4 w-4 mr-2" />Gerar {numVariations > 1 ? `${numVariations} variações` : "imagem"}</>)}
            </Button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
                ...
                <div className="mt-4 w-full max-w-sm bg-secondary/30 rounded-xl border border-border/50 p-4 text-left space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <div><p className="text-[11px] font-medium text-foreground">FLUX.1 Schnell</p><p className="text-[10px] text-muted-foreground">~$0.003/imagem (together.ai)</p></div>
                    </div>
                    <div className="flex items-start gap-2">
                      <Star className="h-3.5 w-3.5 text-purple-400 mt-0.5 flex-shrink-0" />
                      <div><p className="text-[11px] font-medium text-foreground">IP-Adapter / ControlNet</p><p className="text-[10px] text-muted-foreground">~$0.005-0.01/imagem (fal.ai)</p></div>
                    </div>
                    <div className="flex items-start gap-2">
                      <Crown className="h-3.5 w-3.5 text-amber-400 mt-0.5 flex-shrink-0" />
                      <div><p className="text-[11px] font-medium text-foreground">LoRA (treino)</p><p className="text-[10px] text-muted-foreground">~$0.50-2.00 por treino único (fal.ai)</p></div>
                    </div>
                  </div>
            ...
            {results.length > 0 && (
              <div className={cn("grid gap-4", results.length === 1 ? "grid-cols-1 max-w-lg mx-auto" : "grid-cols-2")}>
                {results.map((url, i) => (
