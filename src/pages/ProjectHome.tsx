...
const statusColors: Record<string, string> = {
  draft: "bg-cos-warning/8 text-cos-warning border border-cos-warning/12",
  review: "bg-cos-purple/8 text-cos-purple border border-cos-purple/12",
  approved: "bg-primary/8 text-primary border border-primary/12",
  official: "bg-cos-success/8 text-cos-success border border-cos-success/12",
  archived: "bg-muted/20 text-muted-foreground border border-border/8",
};
  ...
  const statCards = [
    {
      ...
      icon: Zap,
      color: "text-cos-warning",
      bg: "bg-surface-1 border-cos-warning/10",
      iconBg: "bg-cos-warning/10",
    },
    {
      label: "Aprovados",
      ...
      icon: FileCheck,
      color: "text-primary",
      bg: "bg-surface-1 border-primary/10",
      iconBg: "bg-primary/10",
    },
    {
      label: "Oficiais",
      value: stats?.official || 0,
      icon: Star,
      color: "text-cos-success",
      bg: "bg-surface-1 border-cos-success/10",
      iconBg: "bg-cos-success/10",
    },
  ];
    ...
    <div className="p-6 lg:p-8 max-w-5xl space-y-6">
      ...
      <div className="flex items-start justify-between">
        <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-[11px] text-muted-foreground/60 mt-0.5 font-mono-brand tracking-widest uppercase">
            {project?.niche || ""}
          </p>
        </motion.div>
        <div className="flex gap-2">
          <button className="flex items-center gap-1.5 rounded-xl border border-border/10 bg-surface-1 px-3 py-2 text-xs text-muted-foreground/60 hover:bg-surface-2 hover:text-foreground transition-all">
            <Upload className="h-3.5 w-3.5" />Importar
          </button>
          <button onClick={() => navigate(`/project/${projectId}/production`)}
            className="flex items-center gap-1.5 rounded-xl bg-primary px-4 py-2 text-xs font-medium text-primary-foreground hover:bg-primary/90 transition-all shadow-[0_0_20px_-6px_hsl(var(--primary)/0.35)]">
      ...
      {(pendingInsights || 0) > 0 && (
        <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
          className="rounded-2xl border border-cos-purple/12 bg-cos-purple/5 backdrop-blur-sm p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-xl bg-cos-purple/10 flex items-center justify-center">
          ...
          <button onClick={() => openDock()}
            className="rounded-xl bg-cos-purple/10 px-3 py-1.5 text-[11px] font-medium text-cos-purple hover:bg-cos-purple/20 transition-colors">
            Ver
          </button>
        </motion.div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
        className="rounded-2xl border border-border/8 bg-surface-1 backdrop-blur-sm p-5">
        ...
        {brandKit ? (
          <div className="space-y-4">
            <div>
              <p className="text-[9px] font-mono-brand uppercase tracking-[0.2em] text-muted-foreground/50 mb-2">Paleta</p>
              <div className="flex gap-2.5">
                {((brandKit.color_palette as any) || []).map((c: string, i: number) => (
                  <div key={i} className="flex flex-col items-center gap-1.5">
                    <div className="h-8 w-8 rounded-lg border border-border/8 shadow-sm" style={{ backgroundColor: c }} />
                    <span className="text-[8px] font-mono-brand text-muted-foreground/40">{c}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              <p className="text-[9px] font-mono-brand uppercase tracking-[0.2em] text-muted-foreground/50 mb-1.5">Tipografia</p>
              <p className="text-xs">
            ...
            <div className="h-px bg-border/6" />
            <div>
              <p className="text-[9px] font-mono-brand uppercase tracking-[0.2em] text-muted-foreground/50 mb-2">Pilares</p>
              <div className="flex flex-wrap gap-1.5">
                {((brandKit.editorial_line as any) || []).map((p: any, i: number) => (
                  <span key={i} className="rounded-lg bg-primary/6 border border-primary/8 px-2.5 py-1 text-[10px] text-primary/60">
                    {p.pillar || p}
                  </span>
                ))}
              </div>
          ...
          <p className="text-[11px] text-muted-foreground/50">Gere um kit completo com paleta, tipografia e linha editorial baseados no DNA.</p>
        )}
      </motion.div>
      ...
      <div className="grid grid-cols-2 gap-4">
          ...
          {pieData.length > 0 ? (
            <div className="flex items-center gap-4">
              <ResponsiveContainer width="55%" height={140}>
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" outerRadius={55} innerRadius={32} dataKey="value" strokeWidth={0}>
                    {pieData.map((_, idx) => <Cell key={idx} fill={PIE_COLORS[idx % PIE_COLORS.length]} opacity={0.85} />)}
                  </Pie>
                </PieChart>
              </ResponsiveContainer>
              <div className="flex-1 space-y-2.5">
                {pieData.map((d, idx) => (
                  <div key={d.name} className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-muted-foreground/60 flex-1">{d.name}</span>
                    <span className="text-[11px] font-mono-brand font-semibold">{d.value}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center justify-center py-10">
              <p className="text-[11px] text-muted-foreground/40">Sem dados de pipeline</p>
            </div>
          )}
          <div className="mt-3 pt-3 border-t border-border/6">
            <p className="text-[9px] text-muted-foreground/40 text-center font-mono-brand">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              <div className="h-7 w-7 rounded-lg bg-cos-success/8 flex items-center justify-center">
                <DollarSign className="h-3.5 w-3.5 text-cos-success/70" />
              </div>
              <h3 className="text-xs font-semibold font-mono-brand">Consumo</h3>
            </div>
            <span className="rounded-lg bg-cos-success/6 border border-cos-success/8 px-2.5 py-1 text-[9px] font-mono-brand text-cos-success/60">
              ~${totalUSD.toFixed(2)}
            </span>
          </div>
          {barData.length > 0 ? (
            <ResponsiveContainer width="100%" height={130}>
              <BarChart data={barData}>
                <XAxis dataKey="day" tick={{ fontSize: 9, fill: "hsl(215 10% 40%)" }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 9, fill: "hsl(215 10% 40%)" }} axisLine={false} tickLine={false} />
                <Tooltip content={({ active, payload }) => {
                  if (!active || !payload?.[0]) return null;
                  const d = payload[0].payload;
                  return (
                    <div className="rounded-xl border border-border/10 bg-surface-2 backdrop-blur-lg px-3 py-2 text-[10px] shadow-xl">
                      <p className="font-medium font-mono-brand">{d.day}</p>
                      <p className="text-muted-foreground/50">{d.credits}cr · ${d.estimatedUSD}</p>
                    </div>
                  );
                }} />
                <Bar dataKey="credits" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} opacity={0.8} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div className="flex items-center justify-center py-10">
              <p className="text-[11px] text-muted-foreground/40">Sem dados de consumo</p>
            </div>
          )}
          {Object.keys(byOp).length > 0 && (
            <div className="mt-3 pt-3 border-t border-border/6 flex flex-wrap gap-1.5">
              {Object.entries(byOp).map(([op, credits]) => (
                <span key={op} className="rounded-lg bg-surface-2 border border-border/6 px-2 py-0.5 text-[9px] font-mono-brand text-muted-foreground/50">
      ...
      {/* Recent Assets */}
      <div className="rounded-2xl border border-border/8 bg-surface-1 backdrop-blur-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border/6">
          <h2 className="text-xs font-semibold font-mono-brand">Últimos Ativos</h2>
          <button onClick={() => navigate(`/project/${projectId}/library`)} className="flex items-center gap-1 text-[10px] text-primary/50 hover:text-primary transition-colors">
            Ver todos <ArrowRight className="h-3 w-3" />
          </button>
        </div>
        <div className="divide-y divide-border/5">
          {(!recentAssets || recentAssets.length === 0) && (
            <div className="px-5 py-10 text-center text-[11px] text-muted-foreground/40">Nenhum ativo gerado ainda.</div>
          )}
          {recentAssets?.map((asset, i) => (
            <motion.div key={asset.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
              className="flex items-center justify-between px-5 py-3 hover:bg-surface-2/50 transition-colors cursor-pointer group">
              <div className="flex items-center gap-3">
                <span className="rounded-lg bg-surface-2 border border-border/6 px-2 py-0.5 text-[9px] font-mono-brand uppercase tracking-wider text-muted-foreground/50">{asset.output}</span>
                <span className="text-[12px] truncate group-hover:text-primary/80 transition-colors">{asset.title || "Sem título"}</span>
              </div>
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-[9px] text-muted-foreground/40 font-mono-brand">{formatTime(asset.created_at)}</span>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <div>
        <div className="flex items-center gap-2.5 mb-3">
          <div className="h-6 w-6 rounded-lg bg-primary/6 flex items-center justify-center">
            <Bot className="h-3 w-3 text-primary/50" />
          </div>
          <span className="text-[10px] font-mono-brand uppercase tracking-[0.2em] text-muted-foreground/50">Atalhos</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {quickChips.map((chip) => (
            <button key={chip.label} onClick={() => { openDock(); sendMessage(chip.msg); }}
              className="rounded-xl border border-border/8 bg-surface-1 px-3.5 py-2 text-[11px] text-muted-foreground/60 hover:border-primary/15 hover:text-primary/60 hover:bg-primary/5 transition-all">
              {chip.label}
            </button>
          ))}
        </div>
